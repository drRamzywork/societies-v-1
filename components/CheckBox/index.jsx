...
//             <input className={styles.nonCategorizedIcon} type="search" />
//           </div>
//         </div>
...
const CheckBox = ({ societiesListSocieties, selectedSocietiesData, setSelectedSocietiesData }) => {
  const [isOpen, setIsOpen] = useState(true);
  const [checkedStates, setCheckedStates] = useState([]);
  ...
  const handleResize = () => {
    setIsOpen(window.visualViewport.width >= 935);
  };
  ...
  const toggleSelectAll = () => {
    const allSelected = checkedStates.every(Boolean);
    setCheckedStates(new Array(societiesListSocieties.length).fill(!allSelected));
    if (allSelected) {
      setSelectedSocietiesData([]);
    } else {
      const allData = societiesListSocieties.flatMap(society => society.societies);
      ...
      <div className={styles.containerCluster}>
        ...
        <div className={styles.pol}>
          <div className={styles.frameParent1}>
            ...
            <div className={styles.parent} onClick={toggleSelectAll}>
              <div>جميع جمعيات المملكة</div>
              <input
                type="checkbox"
                className={styles[`frameInput`]}
                checked={!checkedStates.includes(false)} // Check if all are true
              // onChange={() => handleCheckboxChange(index)}
              />
            </div>

            {societiesListSocieties.map((society, index) => (
              <div key={index} className={styles.parent} onClick={() => handleCheckboxChange(index)}>
